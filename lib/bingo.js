...
const generateBingoGame = () => {
  ...
  const outcomes = [outcome1, outcome2, outcome3, outcome4];
  const prize = calculatePrize(outcomes);
...
const calculatePrize = (outcomes) => {
  if (outcomes.some(outcome => outcome.D && outcome.d)) {
    return 'grandPrize';
  }
  let wins = 0;
  outcomes.forEach(outcome => {
    outcome.rows.forEach(win => win && wins++);
    outcome.cols.forEach(win => win && wins++);
    outcome.D && wins++;
    outcome.d && wins++;
  })
  if (wins > 2) {
